...
@Slf4j
@Component
public class AuthurGatewayRequestFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ServerHttpRequest build = request.mutate().header(AuthurConstant.ZUUL_TOKEN_HEADER, new String(token)).build();
    ...
    private Mono<Void> checkForbidUri(ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (!shouldForward) {
            AuthurResponse authurResponse = new AuthurResponse().message("该URI不允许外部访问");
            return makeResponse(response, authurResponse);
        }
        return null;
    }
